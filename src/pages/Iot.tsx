...
export default function Iot() {
  ...
  return (
    <div className="flex flex-col min-h-screen pb-12">
      <header className="bg-emerald-900 text-white px-5 pt-5 pb-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[17px] font-bold tracking-tight">物联网设备</h1>
        </div>
      </header>
      ...
      {selectedDevice && currentDevice ? (
        <div className="p-4 space-y-6">
          ...
          <div className="bg-white border-2 border-slate-100 rounded-3xl p-6 shadow-sm">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1 font-mono">序列号: {currentDevice.id.toUpperCase()}-X01</p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 pt-6 border-t border-slate-100">
              {Object.entries(currentDevice.lastData).map(([key, value]) => (
                <div key={key} className="text-center group">
                  <p className="text-[10px] text-slate-400 font-bold mb-1">{key === 'humidity' ? '湿度' : key === 'temp' ? '温度' : key === 'ph' ? 'PH值' : key}</p>
          ...
          <div className="grid grid-cols-2 gap-4">
             <button className="bg-emerald-900 text-white p-6 rounded-3xl flex flex-col items-center gap-3 shadow-lg shadow-emerald-900/20 active:scale-95 transition-all">
                <Power size={24} />
                <span className="text-xs font-bold">远程控制</span>
             </button>
             <button className="bg-white border-2 border-slate-100 p-6 rounded-3xl flex flex-col items-center gap-3 shadow-sm active:bg-slate-50 transition-all font-bold text-slate-400">
                <History size={24} />
                <span className="text-xs">历史曲线</span>
             </button>
          </div>
        ...
        <div className="p-4 space-y-4 pt-6">
           <div className="flex justify-between items-baseline mb-2">
             ...
             <span className="text-[10px] font-bold text-emerald-600">实时更新中</span>
           </div>
           {mockDevices.map((device) => (
             ...
             >
                ...
                <div className="flex-1">
                  <h4 className="text-sm font-bold text-slate-800 group-hover:text-emerald-600 transition-colors">{device.name}</h4>
